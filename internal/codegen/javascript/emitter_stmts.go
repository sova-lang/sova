...
func (e *CodeEmitter) emitWiredStub(ctx *codegen.EmitContext, pkg *ir.PackageContext, f *ir.File, s *ir.FuncDeclStmt) {
	...
	case "Redirect":
		...
		sb.WriteString("  }\n")
		sb.WriteString("  if (!__res.ok) { return [{location: '', status: __res.status || 0}, __res.status === 401 ? 1 : __res.status === 403 ? 2 : __res.status === 404 ? 3 : 4]; }\n")
		sb.WriteString("  const __loc2 = __res.headers.get('Location') || '';\n")
		sb.WriteString("  return [{location: __loc2, status: __res.status}, 0];\n")
	case "Html":
		sb.WriteString("  if (!__res.ok) { return [{body: '', status: __res.status || 0}, __res.status === 401 ? 1 : __res.status === 403 ? 2 : __res.status === 404 ? 3 : 4]; }\n")
		sb.WriteString("  const __htmlBody = await __res.text();\n")
		sb.WriteString("  return [{body: __htmlBody, status: __res.status}, 0];\n")
	case "File":
		sb.WriteString("  if (!__res.ok) { return [{data: new Blob(), filename: '', contentType: '', status: __res.status || 0}, __res.status === 401 ? 1 : __res.status === 403 ? 2 : __res.status === 404 ? 3 : 4]; }\n")
	...
	case "Status":
		sb.WriteString(fmt.Sprintf("  if (!__res.ok && __res.status !== 422 && (__res.status < 200 || __res.status >= 500)) { return [{body: __sovaReify(null, %s), status: __res.status || 0}, __res.status === 401 ? 1 : __res.status === 403 ? 2 : __res.status === 404 ? 3 : 4]; }\n", returnDesc))
	...
	default:
		sb.WriteString(fmt.Sprintf("  if (!__res.ok) { return [__sovaReify(null, %s), __res.status === 401 ? 1 : __res.status === 403 ? 2 : __res.status === 404 ? 3 : 4]; }\n", returnDesc))
		sb.WriteString("  const __data = await __res.json();\n")
