...
// CodeAction returns refactorings and quick fixes available at the cursor or selection. For v1 we offer two:
//
//   - `source.organizeImports` - sort + dedupe the file's `import` statements alphabetically. Available unconditionally as a "Source Action" (editors expose it under "Organize Imports" / Cmd+Shift+O).
//   - Quick-fix `Remove unused import` - fires only when the requested action context already lists a diagnostic on an import line.
//
// Each action returns a `WorkspaceEdit` the editor applies directly; no `Command` round-trip needed.
...
// organizeImportsAction produces a Source Action that rewrites the file's `import` block so entries are sorted alphabetically and duplicates are dropped. Returns nil when the file has 0 or 1 imports (nothing to organize) or when imports are already canonical.
func organizeImportsAction(file *ir.File, src string, docURI uri.URI) *protocol.CodeAction {
	...
	_ = src // src is unused for now but kept on the signature for future "preserve trailing comments" work
...
// removeLineAction proposes a fix that deletes the source line the diagnostic spans (plus its trailing newline). Used as the "remove unused" quick fix: when the compiler reports a symbol as unused, this offers to drop the declaration entirely.
