...
// SemanticTokensFull returns the packed-uint32 token stream the editor consumes for richer-than-grammar highlighting. We emit one token per identifier in the file, classified by the symbol kind the name resolves to: function decls, type/enum/interface decls, parameter names, locals, struct fields, enum members, type-param references, keywords aren't emitted (the editor's grammar already covers them).
...
// SemanticTokensRange returns tokens scoped to a single editor range. The protocol allows servers to ignore the range and return everything; we do exactly that for v1 since our tokens are cheap to build. Editors strip out-of-range entries client-side.
func (s *Server) SemanticTokensRange(ctx context.Context, params *protocol.SemanticTokensRangeParams) (*protocol.SemanticTokens, error) {
	return s.SemanticTokensFull(ctx, &protocol.SemanticTokensParams{TextDocument: params.TextDocument})
}

// semToken is one classification of a source span. The encoder needs all four fields packed into the LSP's delta-encoded 5-uint32-per-token format; this struct captures them before encoding.
...
// semanticTokenTypeIndex maps each `SemanticTokenTypes` value the server emits to its index in the legend the client received during `initialize`. Order here MUST match `semanticTokenLegend` in lifecycle.go.
const (
	tokFunction      = 0
	tokMethod        = 1
	tokVariable      = 2
...
// semanticTokenLegend is the ordered list of token type names the client receives during `initialize`. Its indices match the constants above.
...
// collectSemanticTokens walks every top-level decl in the file, recursing into bodies, and produces one semToken per identifier. The output is sorted by position so the delta encoder can iterate it linearly.
...
func semStmt(s ir.Stmt, out *[]semToken) {
	...
	case *ir.ImportStmt:
		// Alias is a namespace token spanning the entire path's final segment area.
		if n.Alias != "" {
...
func semExpr(e ir.Expr, out *[]semToken) {
	...
	case *ir.EnumValueExpr:
		// Emit the enum container and case name as two tokens; spans on EnumValueExpr aren't fine-grained enough to split, so the whole expr gets one enum-member token via the node's own span.
		emitSem(out, n.Span(), tokEnumMember)
	}
}

// encodeSemanticTokens flattens the sorted semToken slice into the LSP's `[deltaLine, deltaStart, length, tokenType, tokenModifiers]` packed uint32 format. Each token's `deltaLine` is the line delta from the previous token (or absolute when it's the first); `deltaStart` is the column delta within the same line (or absolute when on a new line).
