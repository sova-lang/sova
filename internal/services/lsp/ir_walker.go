...
// walkStmt descends into a statement looking for the cursor. Returns true when a target was identified and populated `t`; the walk short-circuits on first hit so the deepest matching node wins. Only statements that can plausibly contain identifiers at user-visible positions are handled - declaration shells, control flow, expression statements. New statement kinds added later just need a case here.
...
// walkExpr descends into an expression looking for the cursor. Mirrors walkStmt: short-circuits on first hit, captures the most specific information available (resolved symbol when present, field name when on a member access).
...
// setSymTarget records a resolved symbol hit on the cursor target with its kind and source span. Used by both walkStmt and walkExpr whenever we land on a name reference or a declaration name.
...
// walkTypeRef descends into a `TypeRef` looking for a custom-type name (and optional qualifier) that contains the cursor. When the cursor is on `Foo` in `let x: pkg.Foo<T>`, the cursor target is populated with `cursorKindTypeRef` plus the name + qualifier so definition lookup can resolve it to the type's declaration. Walks element / key / value / tuple / func-param / generic-arg subtrees so nested generic references like `option<List<MyType> >` participate.
