...
var (
	...
	ErrAssetNotConst                  = templateErr(CategorySemantic, "`@asset` requires a `const` declaration (got `let`); the asset URL is fixed at build time")
	ErrAssetBadType                   = templateErr(CategorySemantic, "`@asset` on '%s' requires a declared type of `string` (the public URL is the const's value); got %s")
	ErrAssetBadPath                   = templateErr(CategorySemantic, "`@asset` path must be a relative path string literal (got %s); paths are resolved against the declaring file's directory")
	ErrAssetFileNotFound              = templateErr(CategorySemantic, "`@asset` on '%s' cannot find file %s (resolved to %s)")
	ErrAssetFileTooLarge              = templateErr(CategorySemantic, "`@asset` on '%s' refuses to stage %s (%d bytes); the size cap is %d bytes")
