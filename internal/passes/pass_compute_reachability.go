...
// PassComputeReachability marks every top-level declaration (function, type, variable) that
// transitively reaches a root with SF_Reachable. Codegen passes consult the flag to skip
// unreachable declarations entirely, producing tree-shaken Go and JS output.
//
// Roots: `main()` / `boot()` functions, every `wire`d function, every `test` declaration,
// every top-level VarDeclStmt (initializers run at module load), and every TypeDeclStmt
// flagged IsExtern (the runtime may instantiate them via reflection/wire).
//
// Conservative for v1: marking a TypeDecl reachable marks every method, ctor and cast of that
// type reachable too. A future refinement could prune individual methods per type, but the big
// win is dropping entire wrapper classes the consumer never touched (browserx ships ~130
// types; most consumers reach a handful).
