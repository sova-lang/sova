...
// PassInlineMixins resolves `with` clauses on type declarations by deep-cloning the corresponding mixin's fields and methods into each consuming type. A mixin may be applied to any number of types; each application gets its own AST copy with fresh node IDs.
...
// findMixinViaUsing searches the file-local `using *` / `using { ... }` imports for a mixin declaration matching `name`. Returns the mixin from the imported package together with the import alias when exactly one match exists; otherwise returns false.
...
// knownTypeViaUsing reports whether the unqualified `name` resolves to a type declaration in any of the file-local using-imports.
